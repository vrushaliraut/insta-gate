...
1. **GORM (`gorm.io/gorm`):**
   - _Rejected:_ While GORM provides excellent developer ergonomics and rapid prototyping speed, it relies heavily on
...
2. **Standard `database/sql` with `lib/pq`:**
   - _Rejected:_ `lib/pq` is officially in maintenance mode. The standard library abstraction prevents utilizing
     PostgreSQL-specific wire protocol optimizations, such as direct binary encoding/decoding of `JSONB` and array
     types.
3. **Ent (`entgo.io/ent`):**
   - _Rejected:_ Though more type-safe and performant than GORM due to its code-generation approach, Ent's graph-based
