...
1. **`go-chi/chi` or standard `net/http` ServeMux:**
    * *Rejected:* While 100% standard library compatible, they require significantly more boilerplate to handle common
...
2. **Gin (`gin-gonic/gin`):**
    * *Rejected:* Gin uses a custom `gin.Context` pattern but trails behind Fiber in raw benchmark throughput and memory
