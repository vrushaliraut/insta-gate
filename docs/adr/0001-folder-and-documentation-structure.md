...
- **Scattering docs across service folders (`/backend/README.md`, `/frontend/README.md`):**
  - _Rejected:_ Creates fragmented documentation where cross-cutting architectural context (e.g., RBAC or state transitions) is hard to maintain.
- **External Wiki (Notion/Confluence):**
  - _Rejected:_ Documentation drifts from code when stored externally. Co-locating docs with code ensures version-controlled history aligned with PRs.
